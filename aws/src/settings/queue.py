...
def receiptid_queue_exists(queue,receipt):
    # Receive messages
    response = sqs_client.receive_message(
        QueueUrl=queue,
        MaxNumberOfMessages=10,  # Number of messages to retrieve
        WaitTimeSeconds=2        # Wait for messages to arrive
    )
    ...
    for message in messages:
        if message.get("ReceiptHandle") == receipt:
            return True  # Receipt ID exists

    return False  # Receipt ID does not exist
...
def delete_receipt_message(queue, receipt):
    ...
    response = sqs.delete_message(QueueUrl=queue_url, ReceiptHandle=receipt)
    return response
